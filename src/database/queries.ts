...
export function useDatabase() {
  ...
  const getClasses = useCallback(async (): Promise<ClassModel[]> => {
    return await db.getAllAsync<ClassModel>('SELECT * FROM classes');
  }, [db]);
  ...
  const addClass = useCallback(async (cls: ClassModel) => {
    ...
    await db.runAsync(
      'INSERT INTO classes (id, name, subject, timing, days_of_week, weekly_schedule) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  const addExtraClass = useCallback(async (extra: ExtraClassModel) => {
    await db.runAsync(
      'INSERT INTO extra_classes (id, class_id, date, timing) VALUES (?, ?, ?, ?)',
  ...
  const getExtraClassesByDate = useCallback(async (date: string): Promise<ExtraClassModel[]> => {
    return await db.getAllAsync<ExtraClassModel>(
      'SELECT * FROM extra_classes WHERE date = ?',
      [date]
    );
  }, [db]);
  ...
  const getExtraClassesByClass = useCallback(async (classId: string): Promise<ExtraClassModel[]> => {
    return await db.getAllAsync<ExtraClassModel>(
      'SELECT * FROM extra_classes WHERE class_id = ? ORDER BY date DESC',
      [classId]
    );
  }, [db]);

  const deleteClass = useCallback(async (id: string) => {
    await db.runAsync('DELETE FROM extra_classes WHERE class_id = ?', [id]);
    await db.runAsync('DELETE FROM fees WHERE class_id = ?', [id]);
    await db.runAsync('DELETE FROM attendance WHERE class_id = ?', [id]);
  ...
  const getAllStudents = useCallback(async (): Promise<StudentModel[]> => {
    return await db.getAllAsync<StudentModel>('SELECT * FROM students');
  }, [db]);
  ...
  const markAttendance = useCallback(async (attendance: AttendanceModel) => {
    // Upsert equivalent using SQLite (insert or replace)
    await db.runAsync(
      'INSERT OR REPLACE INTO attendance (id, student_id, class_id, date, status, time) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  const assignFee = useCallback(async (fee: FeeModel) => {
    await db.runAsync(
      'INSERT OR REPLACE INTO fees (id, student_id, class_id, month, amount, status, payment_date) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ...
  const getFeesByClassAndMonth = useCallback(async (classId: string, month: string): Promise<FeeModel[]> => {
    return await db.getAllAsync<FeeModel>(
      'SELECT * FROM fees WHERE class_id = ? AND month = ?',
      [classId, month]
    );
  }, [db]);
  ...
  const getDashboardSummary = useCallback(async () => {
    const totalClassesRes = await db.getFirstAsync<{ count: number }>('SELECT COUNT(*) as count FROM classes');
    ...
    const expectedFeesRes = await db.getFirstAsync<{ total: number }>('SELECT SUM(monthly_fee) as total FROM students');
    const paidFeesAmountRes = await db.getFirstAsync<{ total: number }>('SELECT SUM(amount) as total FROM fees WHERE status = "paid" AND month = ?', [currentMonthStr]);

    // Count of students who have paid
    const paidStudentsRes = await db.getFirstAsync<{ count: number }>('SELECT COUNT(*) as count FROM fees WHERE status = "paid" AND month = ?', [currentMonthStr]);
  ...
  const getClassSummary = useCallback(async (classId: string, date: string, month: string) => {
    const todayAttendanceRes = await db.getAllAsync<{ status: string; count: number }>(
      'SELECT status, COUNT(*) as count FROM attendance WHERE class_id = ? AND date = ? GROUP BY status',
      [classId, date]
    );
    const expectedFeesRes = await db.getFirstAsync<{ total: number }>('SELECT SUM(monthly_fee) as total FROM students WHERE class_id = ?', [classId]);
    const paidFeesRes = await db.getFirstAsync<{ total: number }>('SELECT SUM(amount) as total FROM fees WHERE class_id = ? AND status = "paid" AND month = ?', [classId, month]);
